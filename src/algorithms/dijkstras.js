...
const getNodesInOrder = (grid, start, end)=>{
    ...
    while(queue.length > 0){
        ...
        dir.forEach((ele)=>{
            ...
            if(newNode.row<0 || newNode.row>=grid.length || newNode.col<0 || newNode.col>=grid[0].length || grid[newNode.row][newNode.col].isWall === 1 || grid[newNode.row][newNode.col].isVisited  === 1) return;
